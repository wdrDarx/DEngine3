...
Window::Window(const std::string& name, int width, int height)
{
	...
	//set viewport
	RenderUtils::SetViewport({width, height});
	...
	glfwSetCursorEnterCallback(GetRenderAPI()->GetCurrentContext(), [](GLFWwindow* window, int entered)
	{
		if(entered)
		{ 
			EventWindowCursorEnter event;
			StaticCast<Window>(glfwGetWindowUserPointer(window))->m_EventDispatcher.Dispatch(event);
		}
		else
		{
			EventWindowCursorLeave event;
			StaticCast<Window>(glfwGetWindowUserPointer(window))->m_EventDispatcher.Dispatch(event);
		}
	});
...
void Window::StartFrame()
{
	...
	RenderUtils::SetClearColor({0,0,0});
	RenderUtils::Clear();
}
